...
```{r}
# get "Core" table for the Austrian Nationalrat ------------------------------------------
aut_core <- get_core(legislature = "aut")
glimpse(aut_core)
aut_core %>% count(sex)
male_legislators <- aut_core %>% filter(sex == "male") %>% count()
total_legislators <- count(aut_core)
answer1 <- (male_legislators / total_legislators)*100
paste("The overall share of male legislators is ", answer1, "%")
```
...
```{r}
# YOUR CODE HERE
geo_legislators <- select(aut_core,country,birthplace,deathplace)
mutate(geo_legislators,birthplace == deathplace)
```
